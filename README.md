...
一个 [Claude Code Plugin](https://code.claude.com/docs/en/plugins.md)，目前包含一个 Skill：

| Skill | 触发场景 |
|-------|---------|
| `humanize-doc` | 改写 / 润色 / 重写技术文档、RFC、Design Doc、README，或抱怨"太像 AI 写的""说人话""像资深工程师那样写" |

详情见 [`skills/humanize-doc/SKILL.md`](skills/humanize-doc/SKILL.md)。
...
1. **严谨** — 技术内核（接口、字段、量级）不能丢。
2. **人味** — 把"决策"和"权衡"写出来，而不只是陈述结论。
3. **去机械化** — 删掉"综上所述""赋能""全方位"等 AI 套话。
4. **平衡** — 专业但不端着，口语但不轻浮。

交付前有一份强制 checklist，未通过就重写。
